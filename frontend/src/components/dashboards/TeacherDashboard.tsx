...
import { 
  ...
  Test as TestType, 
  StudentSubmission as StudentSubmissionType,
  AssignmentStatus,
  TestStatus,
  SubmissionStatus 
} from '../../types';
...
const TeacherDashboard: React.FC<TeacherDashboardProps> = ({ user, onLogout }) => {
  ...
  const [enteredMarks, setEnteredMarks] = useState<number>(0);
  ...
  useEffect(() => {
    if (selectedTest) {
      setEnteredMarks(selectedTest.totalMarks);
    }
  }, [selectedTest]);
  ...
  const handleGradeSubmission = (
    submissionId: string, 
    grade: string, 
    feedback: string // Added missing parameter
  ) => {
  ...
  const handleSaveMarks = () => {
    if (selectedTest) {
      const updatedTests = tests.map(test => 
        test.id === selectedTest.id ? { ...test, totalMarks: enteredMarks } : test
      );
      setTests(updatedTests);
  ...
  <div className="p-4 mb-8 flex items-center lg:items-start flex-col lg:flex-row lg:justify-start justify-center text-center lg:text-left">
    <ClipboardCheck className="h-8 w-8 text-[#1a237e] mb-2 lg:mb-0 lg:mr-2" />
